...
async function* createEvents(
	...
	yield {
		...
		sequence_number: 2,
		response: { id: "resp_test", status: "completed", output: [completedItem] },
	} as ResponseStreamEvent;
}
...
function getReplayedReasoning(model: Model<"azure-openai-responses">, assistant: AssistantMessage) {
	...
	return convertResponsesMessages(model, context, new Set(["azure-openai-responses"])).find(
		(item) => item.type === "reasoning",
	);
}

describe("Azure OpenAI Responses reasoning replay", () => {
	it("preserves encrypted_content from output_item.done", async () => {
		...
		await processResponsesStream(
			createEvents(doneItem, { ...doneItem, encrypted_content: "from-response-completed" }),
			output,
	...
	it("backfills encrypted_content when output_item.done omitted it", async () => {
		const model = createModel();
		const output = createOutput(model);
		const doneItem: ResponseReasoningItem = { type: "reasoning", id: "rs_missing", summary: [] };
		await processResponsesStream(
			createEvents(doneItem, { ...doneItem, encrypted_content: "from-response-completed" }),
			output,
			new AssistantMessageEventStream(),
