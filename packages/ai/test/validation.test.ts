...
describe("validateToolArguments", () => {
	...
	it("retains native TypeBox conversion outside nullable unions", () => {
		const tool: Tool = {
			name: "echo",
			description: "Echo tool",
		...
		const toolCall: ToolCall = {
			...
			id: "tool-1",
			name: "echo",
			arguments: { value: null as unknown as string },
		};
	...
	it("preserves nested nullable union values while coercing invalid siblings", () => {
		const { tool, toolCall } = createToolCallWithPlainSchema(
			{
				type: "object",
				properties: {
					optionalCount: { anyOf: [{ type: "number" }, { type: "null" }] },
					count: { type: "number" },
				},
				required: ["optionalCount", "count"],
			} as Tool["parameters"],
			{ optionalCount: null, count: "42" },
		);

		expect(validateToolArguments(tool, toolCall)).toEqual({
			value: { optionalCount: null, count: 42 },
		});
	});
