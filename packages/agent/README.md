# @fleetagent/pi-agent-core

Stateful agent with tool execution and event streaming. Built on `@fleetagent/pi-ai`.

## Installation

```bash
npm install @fleetagent/pi-agent-core
```
...
```typescript
import { Agent, streamProxy } from "@fleetagent/pi-agent-core";
...
```typescript
import { agentLoop, agentLoopContinue } from "@fleetagent/pi-agent-core";
