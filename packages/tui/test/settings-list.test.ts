...
describe("SettingsList", () => {
	it("includes spaces in an active search instead of changing the selected setting", () => {
		...
		for (const character of "Quiet startup") list.handleInput(character);

		assert.deepStrictEqual(changes, []);
		assert.match(list.render(80)[0] ?? "", /Quiet startup/);

		list.handleInput("\r");
		assert.deepStrictEqual(changes, [{ id: "quiet-startup", value: "true" }]);
	});
