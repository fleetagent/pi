...
\frac{
  ...
  \displaystyle 1+\frac{1}{n^2}
	...
	it("composes every multiline matrix family through layout nodes", () => {
		const body = String.raw`a&b\\c&d`;
		const cases: readonly LatexCase[] = [
			[String.raw`\begin{matrix}${body}\end{matrix}`, "a │ b\nc │ d"],
			[String.raw`\begin{smallmatrix}${body}\end{smallmatrix}`, "a │ b\nc │ d"],
			[String.raw`\begin{array}{cc}${body}\end{array}`, "a │ b\nc │ d"],
			[String.raw`\begin{pmatrix}${body}\end{pmatrix}`, "⎛ a │ b ⎞\n⎝ c │ d ⎠"],
			[String.raw`\begin{bmatrix}${body}\end{bmatrix}`, "⎡ a │ b ⎤\n⎣ c │ d ⎦"],
			[String.raw`\begin{Bmatrix}${body}\end{Bmatrix}`, "⎧ a │ b ⎫\n⎩ c │ d ⎭"],
			[String.raw`\begin{vmatrix}${body}\end{vmatrix}`, "│ a │ b │\n│ c │ d │"],
			[String.raw`\begin{Vmatrix}${body}\end{Vmatrix}`, "║ a │ b ║\n║ c │ d ║"],
		];
		for (const [source, expected] of cases) {
			assert.strictEqual(renderLatex(source), expected);
...
\begin{pmatrix}\frac{\sqrt{2}}{2}\\\frac{\sqrt{2}}{2}\end{pmatrix}.`,
	...
	it("normalizes relation, multiplication, and named-operator spacing", () => {
		...
		assert.strictEqual(renderLatex(String.raw`x\times y`), "x × y");
	...
	it("bounds source length and remains usable after rejection", () => {
		assert.strictEqual(renderLatex(`x${" ".repeat(16 * 1024 - 1)}`), "x");
		assertRejectedAndRecovers(`x${" ".repeat(16 * 1024)}`);
	});

	it("bounds parser nesting depth without throwing", () => {
		assert.strictEqual(renderLatex(`${"{".repeat(64)}x${"}".repeat(64)}`), "x");
		assertRejectedAndRecovers(`${"{".repeat(65)}x${"}".repeat(65)}`);
		assert.doesNotThrow(() => renderLatex(`${"{".repeat(10_000)}x${"}".repeat(10_000)}`));
	});

	it("bounds shared display layout nodes", () => {
		const operator = String.raw`\sum_0^1`;
		assert.notStrictEqual(renderLatex(operator.repeat(256), { display: true }), undefined);
		assertRejectedAndRecovers(operator.repeat(257), { display: true });

		const matrix = String.raw`\begin{matrix}x\\x\end{matrix}`;
		assert.notStrictEqual(renderLatex(matrix.repeat(256)), undefined);
		assertRejectedAndRecovers(matrix.repeat(257));
		assert.notStrictEqual(
			renderLatex(String.raw`\frac{${matrix.repeat(254)}}{${matrix}}`, { display: true }),
			undefined,
		);
		assertRejectedAndRecovers(String.raw`\frac{${matrix.repeat(255)}}{${matrix}}`, { display: true });
	});

	it("bounds rendered rows", () => {
		const aligned = (rows: number) =>
			String.raw`\begin{aligned}` +
			Array.from({ length: rows }, () => "x").join(String.raw`\\`) +
			String.raw`\end{aligned}`;
		assert.strictEqual(renderLatex(aligned(200))?.split("\n").length, 200);
		assertRejectedAndRecovers(aligned(201));
	});

	it("rejects reserved internal layout markers", () => {
		for (const marker of ["\u{f0000}", "\u{f0001}", "\u{f0002}", "\u{f0004}", "\u{f0005}"]) {
			assertRejectedAndRecovers(`x${marker}y`, { display: true });
		}
	});

	it("bounds matrix row-column expansion", () => {
		const matrix = (rows: readonly string[]) =>
			String.raw`\begin{matrix}` + rows.join(String.raw`\\`) + String.raw`\end{matrix}`;
		const square = Array.from({ length: 16 }, () => Array.from({ length: 16 }, () => "x").join("&"));
		assert.strictEqual(renderLatex(matrix(square))?.split("\n").length, 16);
		assertRejectedAndRecovers(matrix([...square, square[0] ?? ""]));
		const ragged = [Array.from({ length: 200 }, () => "x").join("&"), ...Array.from({ length: 199 }, () => "x")];
		assertRejectedAndRecovers(matrix(ragged));
	});

	it("bounds rendered terminal-cell width", () => {
		assert.strictEqual(renderLatex("x".repeat(4096))?.length, 4096);
		assertRejectedAndRecovers("x".repeat(4097));
		assert.strictEqual(renderLatex("界".repeat(2048))?.length, 2048);
		assertRejectedAndRecovers("界".repeat(2049));
		assertRejectedAndRecovers(String.raw`\begin{matrix}${"x".repeat(4097)}\\x\end{matrix}`);
	});
});
