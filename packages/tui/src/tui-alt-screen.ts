...
export class TuiAltScreen extends TuiBase implements ViewportTUI {
	...
	protected override onOverlayStackChanged(): void {
		if (this.hasCapturingOverlay()) this.cancelActivePointerInteraction();
	}
	...
	private routeWheel(event: WheelEvent): void {
		...
		if (remaining !== 0 && !contained && !seen.has(primary)) primary.scrollBy(remaining);
	...
	private getScrollbarTargetAt(x: number, y: number): ScrollbarTarget | undefined {
		if (this.hasCapturingOverlay() || !this.currentLayout) return undefined;
	...
	private handleSelectionMouseEvent(event: SgrMouseEvent): void {
		...
		const scrollView =
			!this.hasCapturingOverlay() && this.currentLayout
