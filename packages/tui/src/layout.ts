...
function paintBox(box: LayoutBox, screen: string[], totalWidth: number): void {
	if (box.lines) {
		...
		for (let row = firstRow; row < lastRow; row++) {
			...
			const fillsViewport =
				box.rect.x === 0 && box.rect.width >= totalWidth && box.clip.x === 0 && box.clip.width >= totalWidth;
			if (isImageLine(line)) {
				if (fillsViewport) {
					screen[row] = line;
				} else if (box.clip.x === box.rect.x && box.clip.width === box.rect.width) {
					screen[row] = compositeTuiLine(screen[row] ?? "", line, box.rect.x, box.rect.width, totalWidth);
				}
			} else if (box.clip.width > 0) {
				// A full-width box painting onto an untouched row can retain the
				// source line reference. Recomposition would rebuild the row through
				// ANSI/grapheme segmentation on every frame. Keep over-wide sources
				// on the clipping path so LayoutFrame rows remain viewport-bounded.
				if (fillsViewport && !screen[row] && visibleWidth(line) <= totalWidth) {
					screen[row] = line;
				} else {
	...
	if (box.scrollView && box.scrollContentLines && box.scrollView.scrollTop > 0 && box.rect.height > 0) {
		for (let imageRow = box.scrollView.scrollTop - 1; imageRow >= 0; imageRow--) {
			...
			if (metadata) {
				...
				const hiddenRows = box.scrollView.scrollTop - imageRow + (targetRow - box.rect.y);
				if (hiddenRows < metadata.rows) {
					const clipBottom = Math.min(screen.length, box.rect.y + box.rect.height, box.clip.y + box.clip.height);
					const visibleRows = Math.min(clipBottom - targetRow, metadata.rows - hiddenRows);
					if (visibleRows > 0) {
						const cropped = cropKittyImageLine(imageLine, hiddenRows, visibleRows);
						if (
							box.rect.x === 0 &&
							box.rect.width >= totalWidth &&
							box.clip.x === 0 &&
							box.clip.width >= totalWidth
						) {
