...
| Package | Purpose |
|---------|---------|
| `@fleetagent/pi-coding-agent` | Extension types (`ExtensionAPI`, `ExtensionContext`, events) |
| `typebox` | Schema definitions for tool parameters |
| `@fleetagent/pi-ai` | AI utilities (`StringEnum` for Google-compatible enums) |
| `@fleetagent/pi-tui` | TUI components for custom rendering |
...
```typescript
import { isBashToolResult } from "@fleetagent/pi-coding-agent";
...
```typescript
import { createLocalBashOperations } from "@fleetagent/pi-coding-agent";
...
```typescript
import { withFileMutationQueue } from "@fleetagent/pi-coding-agent";
...
**Important:** Use `StringEnum` from `@fleetagent/pi-ai` for string enums. `Type.Union`/`Type.Literal` doesn't work with Google's API.
...
```typescript
import { keyHint } from "@fleetagent/pi-coding-agent";
...
```typescript
import { Text, Component } from "@fleetagent/pi-tui";
...
import { matchesKey } from "@fleetagent/pi-tui";
...
```typescript
import { highlightCode, getLanguageFromPath } from "@fleetagent/pi-coding-agent";
