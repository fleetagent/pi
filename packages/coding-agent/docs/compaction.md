...
When `compress_context` is active, the provider also receives model-only context metadata after each user message and after each completed batch of tool results. It gives persisted user, assistant tool-call, and tool-result entry IDs, **not** a context percentage; tool-result IDs are for lookup, not compression cut points. These notices are not shown in the TUI or saved to session history, and tool-result batches remain contiguous for provider compatibility. Choose a user or assistant cut point from this metadata. `session_search` defaults to entries in the current model context; use `scope: "branch"` or `scope: "all"` to retrieve older history for reference, but replaced or archived IDs are not valid cut points. The primary agent is prompted to consider compression when a task changes, an investigation or implementation slice finishes, or validation has completed and older raw logs are no longer needed—not at a context-percentage threshold.
