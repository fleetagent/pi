...
The built-in `compress_context` tool lets the agent replace at least two messages in the active conversation with its own summary, independently of automatic or manual compaction. Pass `startEntryId` and `summary` to replace from that entry through the current tail, or add an inclusive `endEntryId` to replace an earlier bounded range and retain later messages. The tool call must be the only call in its assistant message; the replacement takes effect after its result is recorded, so no unmatched tool call or result is sent to the model. A bounded range must include complete tool-call/result groups.

When `compress_context` is active, the provider also receives model-only context metadata after each user message and after each completed batch of tool results. It gives the persisted user, assistant tool-call, and tool-result entry IDs plus approximate context utilization at each point; tool-result IDs are for lookup, not compression cut points. These notices are not shown in the TUI or saved to session history, and tool-result batches remain contiguous for provider compatibility. The estimates are recalculated for each request, may be `?` when usage is unavailable, and do not include the notices themselves. Choose a user or assistant cut point from this metadata. `session_search` defaults to entries in the current model context; use `scope: "branch"` or `scope: "all"` to retrieve older history for reference, but archived IDs are not valid cut points.
Pi branches at the entry preceding `startEntryId`, appends the custom summary, then replays any preserved suffix after `endEntryId` onto the new branch in order with new entry IDs. The TUI immediately replaces the selected transcript range with a green `[compress_context]` summary and refreshes the context-utilization footer. The originals—including the compression tool call and result—remain in the JSONL session on the old branch, accessible through `/tree`, `session_entry_get`, or `session_search` with `scope: "all"`. The active branch keeps its prefix and later messages intact. Compression cannot start at a tool result or at/before the most recent standard compaction entry (including its retained messages); start after that boundary instead. Preserve important user instructions, decisions, and current file state in the summary.
...
The footer's `K<n> C<n>` indicators count successful `CONTINUE` (keep) and `COMPRESS` verdicts for the current agent session. Failed, canceled, and invalid responses are excluded; counts remain visible after disabling detection and do not reset on context compaction. Beside the primary session price, `(detect $X.XXXX)` shows the cumulative reported cost of detector responses, including invalid verdicts, separately from the primary agent's spend. This remains visible after detection is disabled; canceled requests without a response cannot contribute reported cost.
