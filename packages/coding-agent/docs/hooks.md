...
| `UserPromptSubmit` | On raw submitted text, before extension commands/input handlers, skill/rule/template expansion, prompt persistence, or provider submission. Blocking rejects the submission without persisting the prompt; the completed hook execution is still recorded in the audit trail. Context output is inserted once as hidden model-visible custom context. |
