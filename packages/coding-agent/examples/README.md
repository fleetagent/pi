...
Base Docker image for `/sandbox start`. Builds from local release artifacts and runs the packaged `pi --daemon` command with `/workspace` as the mounted workspace.
