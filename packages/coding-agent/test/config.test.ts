...
describe("detectInstallMethod", () => {
	test("detects pnpm from Windows .pnpm install paths", () => {
		setExecPath(
			"C:\\Users\\Admin\\Documents\\pnpm-repository\\global\\5\\.pnpm\\@fleetagent+pi-coding-agent@0.67.68\\node_modules\\@fleetagent\\pi-coding-agent\\dist\\cli.ts",
		);

		expect(detectInstallMethod()).toBe("pnpm");
		expect(getUpdateInstruction("@fleetagent/pi-coding-agent")).toBe(
			"Run: pnpm install -g --ignore-scripts --config.minimumReleaseAge=0 @fleetagent/pi-coding-agent",
		);
	});
	...
	test("does not self-update unknown wrapper installs", () => {
		...
		expect(getSelfUpdateCommand("@fleetagent/pi-coding-agent")).toBeUndefined();
		expect(getUpdateInstruction("@fleetagent/pi-coding-agent")).toBe(
			"Update @fleetagent/pi-coding-agent using the package manager, wrapper, or source checkout that provides this installation.",
		);
	});
	...
	test("self-updates npm installs from custom prefixes", () => {
		...
		const command = getSelfUpdateCommand("@fleetagent/pi-coding-agent");
		...
		expect(command).toEqual({
			...
			args: [
				...
				"--ignore-scripts",
				"--min-release-age=0",
				"@fleetagent/pi-coding-agent",
			],
			display: `npm --prefix ${prefix} install -g --ignore-scripts --min-release-age=0 @fleetagent/pi-coding-agent`,
		});
	});
	...
	test("self-update respects configured npmCommand", () => {
		...
		const command = getSelfUpdateCommand("@fleetagent/pi-coding-agent", ["npm", "--prefix", prefix]);
	...
	test("quotes npm self-update display paths", () => {
		...
		expect(command?.display).toBe(
			`npm --prefix "${prefix}" install -g --ignore-scripts --min-release-age=0 @fleetagent/pi-coding-agent`,
		);
	});

	test("does not infer Windows npm custom prefixes from package paths", () => {
		const packageDir = "C:\\Users\\Admin\\npm prefix\\node_modules\\@fleetagent\\pi-coding-agent";
		process.env.PI_PACKAGE_DIR = packageDir;
		setExecPath(`${packageDir}\\dist\\cli.js`);

		expect(detectInstallMethod()).toBe("npm");
		expect(getUpdateInstruction("@fleetagent/pi-coding-agent")).toBe(
			"Run: npm install -g --ignore-scripts --min-release-age=0 @fleetagent/pi-coding-agent",
		);
	});
	...
	test("self-updates bun global installs from bun pm bin", () => {
		...
		expect(command).toEqual({
			command: "bun",
			args: ["install", "-g", "--ignore-scripts", "--minimum-release-age=0", "@fleetagent/pi-coding-agent"],
			display: "bun install -g --ignore-scripts --minimum-release-age=0 @fleetagent/pi-coding-agent",
		});
	});
	...
	test("self-updates pnpm v11 global installs resolved through the store", () => {
		...
		const packageName = "@fleetagent/pi-coding-agent";
		const globalPackageDir = join(root, "11e9a", "node_modules", "@fleetagent", "pi-coding-agent");
	...
	test("does not self-update when npm install path is not writable", () => {
		...
		expect(getSelfUpdateCommand("@fleetagent/pi-coding-agent")).toBeUndefined();
		expect(getSelfUpdateUnavailableInstruction("@fleetagent/pi-coding-agent")).toContain(
